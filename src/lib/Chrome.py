import skia
from .URL import URL
from .Layout import get_font, linespace
from .Draw import Draw, DrawRect, DrawLine, DrawOutline, DrawText 
...
class Chrome:
    def __init__(self, browser) -> None:
        ...
        self.bottom: int = int(self.urlbar_bottom)
        # Back button
        ...
        # Refresh button
        forward_width = self.font.measureText("\N{clockwise gapped circle arrow}") + 2*self.padding
        self.refresh_rect = skia.Rect.MakeLTRB(
        ...
        # Bookmark button
        self.bookmark_width = self.font.measureText("\N{white star}") + 2*self.padding
        self.bookmark_rect = skia.Rect.MakeLTRB(
    ...
    def tab_rect(self, i: int) -> skia.Rect:
    ...
    def paint(self) -> list[Draw]:
        ...
        cmds.append(DrawRect(
            skia.Rect.MakeLTRB(0, 0, self.browser.dimensions["width"], self.bottom),
        "white"))
        # Underline
        cmds.append(DrawLine(
            0, self.bottom-1, self.browser.dimensions["width"], self.bottom-1,
        "black", 1))
        # Back button
        ...
        ctw = self.font.measureText(connection_type) + 2*self.padding
        cmds.append(DrawText(
        ...
        if self.focus == "address bar":
            ...
            w = self.font.measureText(self.address_bar[:self.cursor_position]) if self.cursor_position > 0 else 0
            cmds.append(DrawLine(
    ...
    def click(self, x: int, y: int) -> None:
        ...
        elif self.refresh_rect.contains(x, y):
            self.browser.active_tab.refresh()
        elif self.bookmark_rect.contains(x, y):
